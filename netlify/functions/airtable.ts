...
/**
 * The handler for fetching project data from Airtable securely.
 * It uses server-side environment variables to make the API call.
 */
const handler: Handler = async (event: HandlerEvent) => {
  // Only allow GET requests for fetching projects.
  ...
  if (!AIRTABLE_API_KEY || !AIRTABLE_BASE_ID || !PROJECTS_TABLE_NAME) {
    ...
    return {
      statusCode: 500,
      body: JSON.stringify({ error: "Server configuration error: Missing Airtable project credentials." }),
    };
  }

  const url = `https://api.airtable.com/v0/${AIRTABLE_BASE_ID}/${PROJECTS_TABLE_NAME}`;
